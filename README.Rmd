...
Goals for the MAMBA model is to identify loci which have non-zero replicable associations with a phenotype based on summary statistics from genome-wide association meta-analysis (GWAMA).  
...
Use the `mod$outliermat` object to identify the "most probable outlier" for for non-replicable SNPs.
...
Efron has written extensively on the connection between empirical bayes posterior probability and false discovery rates, for example here: 
